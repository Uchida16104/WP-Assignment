		...
		<div class="modal-list-wrapper"><ul>
			<?php while (have_posts()) : the_post(); ?>
				<li>
					...
					<a href="#id<?php the_latest_ID(); ?>" class="info-title"><?php the_title(); ?></a>
					<?php
					array_push($arrayID, "#id" . get_the_latest_ID()); ?>
				</li>
			<?php endwhile; ?>
		</ul></div>
		<!--<button onclick="open('?php the_permalink(); ?>', 'decoboco-art', 'noopener noreferrer')">新着情報をみる</button>-->
		<div class="text-contents active-disp active-opac">
			<?php query_posts('posts_per_page=-1&cat=3'); ?>
			<?php 
			while(have_posts()): 
				the_post(); 
			?>
				<div class="text-content" id="id<?php the_latest_ID(); ?>" style="display: flex;">
